...
- Provider outcome semantics invariants (provider callback/error codes with multiple real-world meanings are not mapped to one product cause unless the payload proves that cause; copy, durable reasons, and analytics should either remain neutral or record a verified subcause).
- Client transition bridge invariants (redirect query params, localStorage, and sessionStorage used to bridge provider flows are advisory unless validated against signed/server-persisted state and scoped to the current identity/workspace before driving mutations, notices, or attribution analytics).
